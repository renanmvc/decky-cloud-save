...
rclone_exec = plugin_dir / "bin/rclone"
...
def rclone_command(*args: str):
    """
    Builds an rclone command with the plugin-specific config file.

    Parameters:
    args (str): Additional rclone arguments.

    Returns:
    list[str]: Command suitable for subprocess execution.
    """
    return [str(rclone_exec), "--config", str(rclone_cfg), *[str(x) for x in args]]
